...
def evaluate(instance_data, n_instance, problem_size, ref_point, capacity, eva: callable):
    ...
    for n_ins, (objs, t) in enumerate(zip(Archives, times)):
        ...
        obj_2[n_ins] = t
        final_list.append(np.array(objs))

    mean_f1 = np.mean([np.mean(objs[:, 0]) for objs in final_list])
    mean_f2 = np.mean([np.mean(objs[:, 1]) for objs in final_list])
        
    return np.mean(obj_1), np.mean(obj_2), mean_f1, mean_f2
