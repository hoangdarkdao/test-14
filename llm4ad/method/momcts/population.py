...
class Population:
    ...
    def has_duplicate_function(self, func: str | Function) -> bool:
        ...
        for i in range(len(self._population)):
            ...
            if str(f) == str(func):
                if func.metric_score[0] < f.metric_score[0]:
                    self._population[i] = func
                    return True
                if func.metric_score[0] == f.metric_score[0] and func.metric_score[1] < f.metric_score[1]:
                    self._population[i] = func
                    return True
    ...
    def selection(self, pop: List[Function] = None) -> Function:
        ...
        funcs = [f for f in pop if not np.isinf(np.array(f.metric_score)).any()]
        if len(funcs) > 1:
            a, b = np.random.choice(funcs, size=2, replace=False)
            if dominates(a.metric_score, b.metric_score):
                return a
            elif dominates(b.metric_score, a.metric_score):
                return b
    ...
    def survival(self, pop_size: int = None):
        ...
        pop = [ind for ind in self._population +
               self._next_gen_pop if ind.metric_score is not None]
        if pop_size > len(pop):
            pop_size = len(pop)
            ...
            if len(survivors) + len(front_individuals) <= pop_size:
                ...
                if remaining_slots > 0:
                    front_individuals_scores = np.array(
                        # List[List[float]]
                        [ind.metric_score for ind in front_individuals])
