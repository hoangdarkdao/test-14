...
class MOMCTS_AHD:
    ...
    def _sample_evaluate_register(self, prompt, op: str, func_only=False, max_retries=3):
        for attempt in range(max_retries):
            ...
            print(f"check output score: {score}")
        ...
        func.raw_score = score[2:]
        func.metric_score = score[:2]
        func.evaluate_time = eval_time
        func.algorithm = thought
        func.sample_time = sample_time
        
        print(f"Check raw score: {func.raw_score} and metric: {func.metric_score}")
    ...
    def check_duplicate_obj(self, population, score: list[float, float]):
        for ind in population:
            if np.array_equal(score, ind.individual.metric_score):
                return True
        return False
    ...
    def population_management_s1(self, pop_input: list[Function], size=None):
        ...
        pop = [ind for ind in pop_input if ind.metric_score is not None]
        if not pop:
            return []
        ...
        # Score matrix
        scores = np.array([ind.metric_score for ind in unique_pop])
    ...
    def expand(self, mcts: MCTS, node_set: list[MCTSNode], cur_node: MCTSNode, option: str):
        ...
        if option == 's1':
            ...
            while i < 3:
                ...
                if func is False:
                    ...
                    i += 1
                    continue
                is_valid_func = (func.metric_score is not None) and not self.check_duplicate(
                    node_set, str(func))
            ...
            if func is False:
                ...
                is_valid_func = (func.metric_score is not None)
            ...
            while i < 3:
                ...
                if func is False:
                    ...
                    i += 1
                    continue
                is_valid_func = (func.metric_score is not None) and not self.check_duplicate(node_set, str(func))
                if is_valid_func is False:
                    i += 1
                    continue
                else:
            ...
            while i < 3:  # Retries giới hạn để flash
                ...
                if is_valid_func is False:
                    ...
                    is_dominated = any(dominates(elite.metric_score, func.metric_score) for elite in elites)            
                    if not is_dominated: 
            ...
            print(f"Action: {option}, Elites Obj: {[e.metric_score for e in elites]}, New Obj: {func.metric_score}, Depth: {cur_node.depth + 1}")

            if is_valid_func and np.any(func.metric_score != float('-inf')):
                self._population.register_function(func)
                print(f"Passed score into MCTSNode in expand: {func.metric_score}")
                # func.score here [NHV, runtime]
                runtime = func.metric_score[1]
                runtime_normalized = max(0.0, min(10.0, (runtime / 10.0) * 10.0))
                combined_raw_score = list(func.raw_score) + [runtime_normalized]

                now_node = MCTSNode(func.algorithm, str(func), combined_raw_score, individual=func,
        ...
        if option != 'e1':
            print(
                f"Action: {option}, Father Obj: {cur_node.raw_info.metric_score}, Now Obj: {func.metric_score}, Depth: {cur_node.depth + 1}")
        else:
            if self.check_duplicate_obj(node_set, func.metric_score):
                print(
                    f"Duplicated e1, no action, Father is Root, Abandon Obj: {func.metric_score}")
            else:
                print(
                    f"Action: {option}, Father is Root, Now Obj: {func.metric_score}")
    ...
    def run(self):
        ...
        self.mcts = MCTS('Root', num_objectives=3, alpha=self.alpha,
        ...
        for indiv in self._population.population:
            runtime = indiv.metric_score[1] if hasattr(indiv, "metric_score") else 0.0
            runtime_norm = min(10.0, max(0.0, (runtime / 10.0) * 10.0))
            combined_raw_score = list(indiv.raw_score) + [runtime_norm]
