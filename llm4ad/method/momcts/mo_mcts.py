...
class MCTS:
    ...
    def _calculate_hypervolume(self, front: List[List[float, float]]) -> float: 
        ...
        z_ideal = np.array([0, 0, 0]) 
        z_nadir = np.array([1.5, 1.5, 1.5]) 
    ...
    def _calculate_multi_objective_ucb(self, child: MCTSNode, parent_visits: int) -> List[float]:
        """
        Multi-objective UCB calculation (minimization problem).
        Each objective is normalized independently within its bounds.
        Lower value = better performance (for runtime and other objectives).
        """
        avg_reward = []
        for i in range(self.num_objectives):
            if child.visits > 0:
                ...
                normalized_vals = [
                    (r[i] - self.lower_bound[i]) / (self.upper_bound[i] - self.lower_bound[i] + self.epsilon)
                    for r in child.rewards_collected
                ]
                # clip normalization range to avoid noise
        ...
        # smaller is better (minimization) → UCB = mean - exploration
        exploration_term = self.exploration_constant_0 * math.sqrt(
            math.log(parent_visits + 1) / (child.visits + self.epsilon)
        )

        weights = [1.0] * (self.num_objectives - 1) + [0.5]  

        ucb_vector = [
            avg_reward[i] - weights[i] * exploration_term
            for i in range(self.num_objectives)
        ]
        return ucb_vector
