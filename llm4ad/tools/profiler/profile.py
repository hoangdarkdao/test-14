...
class ProfilerBase:
    ...
    def _write_json(self, prompt: str, function: Function, program: str, *, record_type: Literal['history', 'best'] = 'history',
        ...
        content = {
            ...
            'metric_score': function.metric_score,
            'raw_score': function.raw_score,
            'program': program,
        }
    ...
    def _record_and_print_verbose(self, prompt:str, function, program='', *, resume_mode=False):
        ...
        if self._num_objs < 2:
            if metric_score is not None and metric_score > self._cur_best_program_score:
                self._cur_best_function = function
                self._cur_best_program_score = metric_score
            ...
            if metric_score is not None:
                for i in range(self._num_objs):
                    if metric_score[i] < self._cur_best_program_score[i]:
                        self._cur_best_function[i] = function
                        self._cur_best_program_score[i] = metric_score[i]
        ...
        if not resume_mode:
            ...
            if self._log_style == 'complex':
                ...
                print(f'------------------------------------------------------')
                print(f'Score        : {str(metric_score)}')
                ...
                if metric_score is None:
                    ...
                    if self._num_objs < 2:
                        print(
                            f'Sample{self._num_samples}: Score={metric_score: .3f}     Cur_Best_Score={self._cur_best_program_score: .3f}')
                    else:  # TODO: MEoH: only support 2 objs
                        print(
                            f'Sample{self._num_samples}: Score=[{metric_score[0]: .3f}, {metric_score[1]: .3f}]     Cur_Best_metric_score=[{self._cur_best_program_score[0]: .3f}, {self._cur_best_program_score[1]: .3f}]')
